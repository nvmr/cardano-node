    ...
    let witnessesByron = map (Api.makeShelleyBootstrapWitness nw txbody) sksByron
        ...
        witnessesShelley =
          map (Api.makeShelleyKeyWitness txbody) sksShelley
            ++ map (makeShelleyScriptWitness . makeMultiSigScript) scsShelley
...
runTxSign (TxBodyFile txbodyFile) skFiles Nothing (TxFile txFile) = do
    ...
    witnesses <- case partitionSomeWitnesses $ map categoriseSomeWitness sks of
                   ([], sksShelley, scsShelley) ->
                     return $
                       map (Api.makeShelleyKeyWitness txbody) sksShelley
                         ++ map (Api.makeShelleyScriptWitness . Api.makeMultiSigScript) scsShelley
                   (_, _, _) -> throwError ShelleyTxCmdMissingNetworkId
